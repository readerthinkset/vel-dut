...
IMPROVED VERSION: Better backgrounds, English categories, no repeats, VELOCITY DUTCH branding
...
NATIVE_VOICE = "nl-NL-MaartenNeural"
...
IMPORTANT: Dutch text must be clean - no slashes, no multiple versions."""
            ...
            for p in phrases:
                ...
                if "dutch" not in p:
                    alt_keys = ["dutch_text", "native", "translation", p.get("language", "")]
                    for k in alt_keys:
                        if k in p:
                            p["dutch"] = p.pop(k)
                            break
...
def generate_all_audio(phrases: list, output_dir: str):
    ...
    for i, phrase in enumerate(phrases):
        ...
        print(f"    NL: {phrase['dutch']}")

        nat_success = asyncio.run(generate_audio_with_retries(phrase["dutch"], NATIVE_VOICE, str(native_file)))
        if nat_success:
            print(f"    - Dutch: {native_file.name}")
        else:
            print(f"    - Dutch: SILENCE FALLBACK (TTS failed)")
        ...
        print(f"    Total: {total_duration:.2f}s (EN: {en_duration:.2f}s + pause: {pause_between}s + NL: {nat_duration:.2f}s)")
...
def create_video_from_images_audio(image_files: list, audio_files: list, combined_audio: str, output_file: str):
    ...
    for i, (img_path, audio_info) in enumerate(zip(image_files, audio_files)):
        ...
        print(f"  Image {i+1}/{len(image_files)}: {duration:.2f}s (EN: {audio_info.get('en_duration', 0):.1f}s + NL: {audio_info.get('nat_duration', 0):.1f}s)")
...
def generate_reel(category_english: str = None):
    ...
    print(f"  Branding: VELOCITY DUTCH")
    print(f"{'='*80}\n")
...
if __name__ == "__main__":
    print("\n" + "="*80)
    print(f"VELOCITY DUTCH - FACEBOOK REELS AUTOMATION")
    print("="*80)
    print("\nFEATURES:")
    ...
    print(f"  - VELOCITY DUTCH branding at bottom")
