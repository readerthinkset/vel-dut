"""
YouTube Upload Script - VELOCITY DUTCH
"""
import os
...
def generate_video_metadata(category, num_phrases, phrases=None):
    title = f"Dutch Learning: {num_phrases} Essential {category} Phrases"
    lines = [f"Learn Dutch with VELOCITY DUTCH!", "", f"Category: {category}", "", f"Master Dutch one phrase at a time! Today's {category} lesson:", ""]
    if phrases:
        for i, p in enumerate(phrases[:5], 0):
            lines.append(f"{i+1}. {p['english']}")
            lines.append(f"   {p.get('dutch', '')}")
            lines.append(f"   [{p.get('transliteration', '')}]")
            lines.append("")
    lines.extend(["Tip: Repeat each phrase out loud 3 times!", "Like this video if you learned something new!", "Subscribe for daily Dutch lessons!", "", f"#LearnDutch #DutchLessons #DutchForBeginners #LanguageLearning", f"#Dutch #Education #Tutorial #DailyDutch #{category.replace(' ', '')}", f"#VELOCITYDUTCH #DutchPhrases #SpeakDutch"])
    tags = [f"learn dutch", f"dutch lessons", f"dutch for beginners", f"dutch phrases", "language learning", f"speak dutch", category.lower(), "education", f"daily dutch", "velocity dutch", f"dutch learning"]
    return title, "\n".join(lines), tags

def upload_to_youtube(video_path, title, description, tags=None, category_id='22'):
    if tags is None: tags = ['education', 'language learning', '#Dutch']
...
def main():
    ...
    try: upload_to_youtube(video_path=video_file, title=f"Learn Dutch Daily", description=f"#shorts #learndutch #dutch #language #education", tags=['#Dutch', 'education', 'language learning'], category_id='22')
